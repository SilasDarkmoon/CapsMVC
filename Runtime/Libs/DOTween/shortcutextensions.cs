...
//            return tot + DOTween.Kill(target);
        ...
        // ------------------------- ADD ----------------------
        /// <summary>Tweens a RawImage's uvRect X to the given value.
        /// Also stores the RawImage as the tween's target so it can be used for filtered operations</summary>
        /// <param name="endValue">The end value to reach</param><param name="duration">The duration of the tween</param>
        /// <param name="snapping">If TRUE the tween will smoothly snap all values to integers</param>
        public static Tweener DOUVRectX(this RawImage target, float endValue, float duration, bool snapping = false)
        {
            return DOTween.To(() => target.uvRect.x, x => target.uvRect = new Rect(x, target.uvRect.y, target.uvRect.width, target.uvRect.height), endValue, duration)
                .SetOptions(snapping).SetTarget(target);
        }
        // ------------------------- ADD ----------------------
...
.SetOptions(snapping).SetTarget(target);
        ...
        /// <summary>Tweens a Text's fontSize to the given value.
        public static Tweener DOTextFontSize(this Text target, int endValue, float duration)
        {
